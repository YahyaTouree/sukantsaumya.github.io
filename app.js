...
// Populate
function fillSkills(){ const c=document.getElementById('skills-container'); if(!c) return; c.innerHTML=''; skillsData.forEach(s=>{ const el=document.createElement('div'); el.className='glass-card text-center p-4 reveal'; el.innerHTML=`<i class="${s.icon} text-3xl mb-2 text-accent"></i><p>${s.name}</p>`; c.appendChild(el); }); }
function fillProjects(){ const g=document.getElementById('projects-grid'); if(!g) return; g.innerHTML=''; projectsData.forEach(p=>{ const wrapper=document.createElement('div'); wrapper.className='project-card h-96 relative reveal'; wrapper.innerHTML=`<div class="project-inner relative w-full h-full"><div class="project-front glass-card p-6 relative"><div class="absolute inset-0 bg-cover opacity-10" style="background-image:url('${p.image}')"></div><div class="relative z-10 flex flex-col h-full justify-between"><div><h3 class="text-2xl font-bold text-accent">${p.title}</h3><p class="text-sm text-gray-300 mb-3">${p.subtitle}</p><p class="text-gray-400">${p.description}</p></div><div class="flex justify-between items-center"><span class="text-sm text-accent">Hover for details →</span><button class="ai-explain-btn bg-accent/20 text-accent px-2 py-1 rounded" data-title="${encodeHTML(p.title)}" data-details="${encodeHTML(p.description)}">✨ Ask AI</button></div></div></div><div class="project-back glass-card p-6 flex flex-col justify-between"><div><h4 class="text-xl font-bold mb-4">Key Metrics</h4><div class="grid grid-cols-2 gap-4 text-center">${p.details.map(d=>`<div><p class="text-2xl font-bold text-accent">${d.value}${d.unit||''}</p><p class="text-xs text-gray-400">${d.label}</p></div>`).join('')}</div></div><div class="flex flex-wrap gap-2 mt-3">${p.tags.map(t=>`<span class="bg-gray-700 text-gray-300 text-xs px-2 py-1 rounded">${t}</span>`).join('')}</div></div></div>`; g.appendChild(wrapper); }); }
